...
class AudioClassifierHelper(
    ...
    private fun classifyAudioAsync(audioRecord: AudioRecord) {
        ...
        var sum = 0f
        val length = audioData.bufferLength
        for (i in 0 until length) {
            val sample = floatArray[i]
            sum += sample * sample
        }
        val rms = Math.sqrt((sum / length).toDouble()).toFloat()
        // Normalize RMS to 0.0 - 1.0 (roughly, assuming max is around 0.5 for typical mic)
        val normalizedAmplitude = (rms * 2.0f).coerceIn(0f, 1f)
        amplitudeListener?.invoke(normalizedAmplitude)
